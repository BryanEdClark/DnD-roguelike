...
test.describe('Character Creation and Management', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('#playerTools', { state: 'visible' });
  });
  ...
  test('should select species and populate subspecies options', async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    const subspeciesSelect = page.locator('#charSpeciesType');
    await expect(subspeciesSelect).toBeEnabled();

    const subspeciesOptions = await page.$$eval('#charSpeciesType option', options =>
  ...
  test('should calculate ability modifiers correctly', async ({ page }) => {
    ...
    const strMod = await page.textContent('#str-mod');
    ...
    const dexMod = await page.textContent('#dex-mod');
    expect(dexMod).toBe('-1');
  });

  test('should calculate proficiency bonus based on level', async ({ page }) => {
    // Level 1-4: +2
    await page.selectOption('#charLevel', '4');
    await page.waitForTimeout(300);
    let profBonus = await page.textContent('#profBonus');
    expect(profBonus).toBe('2');

    // Level 5-8: +3
    await page.selectOption('#charLevel', '7');
    await page.waitForTimeout(300);
    profBonus = await page.textContent('#profBonus');
    expect(profBonus).toBe('3');

    // Level 17-20: +6
    await page.selectOption('#charLevel', '20');
    await page.waitForTimeout(300);
    profBonus = await page.textContent('#profBonus');
  ...
  test('should auto-populate class traits', async ({ page }) => {
    ...
    await page.click('button:has-text("Auto-Populate Class Traits")');
    await page.waitForTimeout(1000);

    // Should have Rage trait
    await expect(page.locator('.trait-name:has-text("Rage")')).toBeVisible();

    // Should have Unarmored Defense
    await expect(page.locator('.trait-name:has-text("Unarmored Defense")')).toBeVisible();
  });

  test('should auto-populate species traits', async ({ page }) => {
    await page.selectOption('#charSpecies', 'Dragonborn');
    await page.selectOption('#charSpeciesType', 'Red Dragon');
    await page.waitForTimeout(300);

    // Click auto-populate button
    await page.click('button:has-text("Auto-Populate Species Traits")');
    await page.waitForTimeout(1000);

    // Should have Breath Weapon
    await expect(page.locator('.trait-name:has-text("Breath Weapon")')).toBeVisible();

    // Should have Damage Resistance
    await expect(page.locator('.trait-name:has-text("Damage Resistance")')).toBeVisible();
  });

  test('should apply Unarmored Defense to AC', async ({ page }) => {
    await page.selectOption('#charClass', 'Barbarian');
    await page.selectOption('#charLevel', '1');
  ...
  test('should calculate attack bonuses correctly', async ({ page }) => {
    ...
    const meleeBonus = await page.textContent('#meleeAttackBonus');
    expect(meleeBonus).toBe('+7');

    // Ranged: +3 (prof) + 2 (DEX) = +5
    const rangedBonus = await page.textContent('#rangedAttackBonus');
    expect(rangedBonus).toBe('+5');

    // Finesse: +3 (prof) + 4 (higher of STR/DEX) = +7
    const finesseBonus = await page.textContent('#finesseAttackBonus');
  ...
  test('should save character data persistently', async ({ page }) => {
    ...
    await page.selectOption('#charLevel', '10');
  ...
  test('should delete character with confirmation', async ({ page }) => {
    ...
    // Confirm deletion
    await page.click('#deleteCharacterModal button:has-text("Delete")');
    await page.waitForTimeout(500);

    // Character name should be cleared
    const charName = await page.inputValue('#charName');
    expect(charName).toBe('');
  });
});
...
test.describe('Saving Throws and Skills', () => {
  ...
  test('should calculate saving throw bonuses', async ({ page }) => {
    ...
    const strSaveBonus = await page.textContent('#strSaveBonus');
  ...
  test('should mark class proficient saves automatically', async ({ page }) => {
    ...
    const strSaveProfChecked = await page.isChecked('#strSaveProf');
    const conSaveProfChecked = await page.isChecked('#conSaveProf');
  ...
  test('should calculate skill bonuses with proficiency', async ({ page }) => {
    ...
    await page.check('input[data-skill="stealth"]');
    await page.waitForTimeout(300);

    // Bonus should be +6 (3 mod + 3 prof)
    const stealthBonus = await page.textContent('[data-skill-bonus="stealth"]');
